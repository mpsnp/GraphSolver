...
std::vector<cv::Point> GraphSolver::_findInputVectors()
{
    ...
    inverseThresholdedImage.copyTo(withoutAxes);
    ...
    for( size_t i = 0; i < axes.size(); i++)
    {
        ...
        pt2.x = cvRound(x0 - 1000*(-b));
        pt2.y = cvRound(y0 - 1000*(a));
        line(withoutAxes, pt1, pt2, Scalar(0,0,0), 15, CV_AA);
    }
    
    vector< Vec4i > hierarchy;
    findContours(withoutAxes, _graphicPoints, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE);
...
std::vector<double> GraphSolver::calculatePolynomOnImage(cv::Mat AImage, int ADegree, double AThreshold)
{
    ...
    std::vector<double> result = _findPolynom(_transformInputVectors(_findInputVectors()), ADegree);
